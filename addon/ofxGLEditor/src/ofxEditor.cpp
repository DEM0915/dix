...
int ofxEditor::s_charHeight = 0.5;
...
bool ofxEditor::loadFont(const string &font, int size) {
	...
	if(s_font->load(font, size)) {
		...
		s_cursorWidth = MAX(floor(s_charWidth*0.1), 2);
        
		s_autoFocusError = MAX(floor(s_charHeight*0.1), 1); // make sure the error space is proportional to the glyph size
	}
}
